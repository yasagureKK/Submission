...
int CTitle::m_nChangeTransition = 0;
...
HRESULT CTitle::Init(D3DXVECTOR3 pos, D3DXVECTOR3 size)
{
	...
	//+------------+
	// タイトルロゴ
	m_apUi[0] = CUi::Create(D3DXVECTOR3(SCREEN_WIDTH / 2, TITLE_LOGO_POS_Y, 0.0f),
	...
	if (pFade->GetFade() == CFade::FADE_NONE)
	{
		for (int nNum = 0; nNum < MAX_PLAYER_NUM; nNum++)
		{
				...
				pInputPad->GetButtonTrigger(XINPUT_GAMEPAD_START, nNum) == true)
			{
				...
				m_nChangeTransition = 0;
			}
		}

		// 自動的に遷移
		if (m_nFadeCount >= 2340)
		{
			// BGMの停止
			pSound->Stop(CSound::SOUND_LABEL_TITLE);

			// 変数の値によって遷移先を変更
			if (m_nChangeTransition == 0)
			{
				// ランキング画面に遷移
				pFade->SetFade(CManager::MODE_RANKING, D3DCOLOR_RGBA(0, 0, 0, 0));

				m_nChangeTransition = 1;
			}
			else if (m_nChangeTransition == 1)
			{
				// チュートリアル画面に遷移
