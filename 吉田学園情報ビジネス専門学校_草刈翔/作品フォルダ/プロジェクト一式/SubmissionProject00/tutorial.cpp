...
HRESULT CTutorial::Init(D3DXVECTOR3 pos, D3DXVECTOR3 size)
{
	...
	// 前のモードがメニュー画面またはタイトル画面だったら
	if (CManager::GetBeforeMode() == CManager::MODE_MENU || CManager::GetBeforeMode() == CManager::MODE_TITLE)
	{
		m_bExplanation = true;
	}
	else
	...
	//| プレイヤーの生成 |
	//+------------------+
	m_pPlayer = CPlayer::Create(D3DXVECTOR3(GAME_PLAYER_START, 0.0f, 0.0f), D3DXVECTOR3(0.0f, D3DX_PI, 0.0f), 0);

	//+------------------------------------------------+
	//| メッシュフィールドの生成・テクスチャの割り当て |
	//+------------------------------------------------+
	// 草原
	m_pMeshField[0] = CMeshField::Create(D3DXVECTOR3(GAME_PLAYER_START, 0.0f, 0.0f), D3DXVECTOR3(2000.0f, 0.0f, 800.0f), D3DXVECTOR3(0.0f, 0.0f, 0.0f), 20, 5, CMeshField::MESHFIELD_TYPE_RIGHT_LOOP_START);
	m_pMeshField[0]->BindTexture(CManager::GetTexture()->GetTexture(CTexture::TEX_TYPE_FIELD00));
	m_pMeshField[1] = CMeshField::Create(D3DXVECTOR3(GAME_PLAYER_START + 2000.0f, 0.0f, 0.0f), D3DXVECTOR3(2000.0f, 0.0f, 800.0f), D3DXVECTOR3(0.0f, 0.0f, 0.0f), 20, 5, CMeshField::MESHFIELD_TYPE_RIGHT_LOOP_START);
	m_pMeshField[1]->BindTexture(CManager::GetTexture()->GetTexture(CTexture::TEX_TYPE_FIELD00));
	m_pMeshField[2] = CMeshField::Create(D3DXVECTOR3(1000.0f, 0.0f, 0.0f), D3DXVECTOR3(2000.0f, 0.0f, 800.0f), D3DXVECTOR3(0.0f, 0.0f, 0.0f), 20, 5, CMeshField::MESHFIELD_TYPE_RIGHT_LOOP);
	m_pMeshField[2]->BindTexture(CManager::GetTexture()->GetTexture(CTexture::TEX_TYPE_FIELD00));
	m_pMeshField[3] = CMeshField::Create(D3DXVECTOR3(3000.0f, 0.0f, 0.0f), D3DXVECTOR3(2000.0f, 0.0f, 800.0f), D3DXVECTOR3(0.0f, 0.0f, 0.0f), 20, 5, CMeshField::MESHFIELD_TYPE_RIGHT_LOOP);
	m_pMeshField[3]->BindTexture(CManager::GetTexture()->GetTexture(CTexture::TEX_TYPE_FIELD00));
	// 土；スタート
	m_pMeshField[4] = CMeshField::Create(D3DXVECTOR3(-119.0f, 0.0f, 0.0f), D3DXVECTOR3(200.0f, 0.0f, 200.0f), D3DXVECTOR3(0.0f, 0.0f, 0.0f), 1, 1, CMeshField::MESHFIELD_TYPE_NONE);
	m_pMeshField[4]->BindTexture(CManager::GetTexture()->GetTexture(CTexture::TEX_TYPE_FIELD01));
	// 土
	m_pMeshField[5] = CMeshField::Create(D3DXVECTOR3(980.0f, 0.0f, 0.0f), D3DXVECTOR3(2000.0f, 0.0f, 200.0f), D3DXVECTOR3(0.0f, 0.0f, 0.0f), 10, 1, CMeshField::MESHFIELD_TYPE_RIGHT_LOOP);
	m_pMeshField[5]->BindTexture(CManager::GetTexture()->GetTexture(CTexture::TEX_TYPE_FIELD02));
	m_pMeshField[6] = CMeshField::Create(D3DXVECTOR3(2980.0f, 0.0f, 0.0f), D3DXVECTOR3(2000.0f, 0.0f, 200.0f), D3DXVECTOR3(0.0f, 0.0f, 0.0f), 10, 1, CMeshField::MESHFIELD_TYPE_RIGHT_LOOP);
	m_pMeshField[6]->BindTexture(CManager::GetTexture()->GetTexture(CTexture::TEX_TYPE_FIELD02));
	...
	m_pMeshCylinder[1]->BindTexture(CManager::GetTexture()->GetTexture(CTexture::TEX_TYPE_CLOUD00));
	...
	m_pMeshCylinder[2]->BindTexture(CManager::GetTexture()->GetTexture(CTexture::TEX_TYPE_CLOUD01));

	//+----------------------------------------------+
	//| メッシュウォールの生成・テクスチャの割り当て |
	//+----------------------------------------------+
	// 山
	m_pMeshWall[0] = CMeshWall::Create(D3DXVECTOR3(GAME_PLAYER_START, -450.0f, 600.0f), D3DXVECTOR3(20000.0f, 2000.0f, 0.0f), D3DXVECTOR3(0.0f, 0.0f, 0.0f), 1, 1, CMeshWall::MESHWALL_TYPE_RIGHT_LOOP);
	m_pMeshWall[0]->BindTexture(CManager::GetTexture()->GetTexture(CTexture::TEX_TYPE_WALL_MOUNTAIN_00));
	m_pMeshWall[1] = CMeshWall::Create(D3DXVECTOR3(GAME_PLAYER_START + 20000.0f, -450.0f, 600.0f), D3DXVECTOR3(20000.0f, 2000.0f, 0.0f), D3DXVECTOR3(0.0f, 0.0f, 0.0f), 1, 1, CMeshWall::MESHWALL_TYPE_RIGHT_LOOP);
	m_pMeshWall[1]->BindTexture(CManager::GetTexture()->GetTexture(CTexture::TEX_TYPE_WALL_MOUNTAIN_00));
	// 山
	m_pMeshWall[2] = CMeshWall::Create(D3DXVECTOR3(GAME_PLAYER_START, -350.0f, 500.0f), D3DXVECTOR3(15000.0f, 1500.0f, 0.0f), D3DXVECTOR3(0.0f, 0.0f, 0.0f), 1, 2, CMeshWall::MESHWALL_TYPE_RIGHT_LOOP);
	m_pMeshWall[2]->BindTexture(CManager::GetTexture()->GetTexture(CTexture::TEX_TYPE_WALL_MOUNTAIN_01));
	m_pMeshWall[3] = CMeshWall::Create(D3DXVECTOR3(GAME_PLAYER_START + 15000.0f, -350.0f, 500.0f), D3DXVECTOR3(15000.0f, 1500.0f, 0.0f), D3DXVECTOR3(0.0f, 0.0f, 0.0f), 1, 2, CMeshWall::MESHWALL_TYPE_RIGHT_LOOP);
	m_pMeshWall[3]->BindTexture(CManager::GetTexture()->GetTexture(CTexture::TEX_TYPE_WALL_MOUNTAIN_01));
	// 森
	m_pMeshWall[4] = CMeshWall::Create(D3DXVECTOR3(GAME_PLAYER_START, -10.0f, 200.0f), D3DXVECTOR3(6400.0f, 300.0f, 0.0f), D3DXVECTOR3(0.0f, 0.0f, 0.0f), 1, 2, CMeshWall::MESHWALL_TYPE_RIGHT_LOOP);
	m_pMeshWall[4]->BindTexture(CManager::GetTexture()->GetTexture(CTexture::TEX_TYPE_WALL_FOREST));
	m_pMeshWall[5] = CMeshWall::Create(D3DXVECTOR3(GAME_PLAYER_START + 6400.0f, -10.0f, 200.0f), D3DXVECTOR3(6400.0f, 300.0f, 0.0f), D3DXVECTOR3(0.0f, 0.0f, 0.0f), 1, 2, CMeshWall::MESHWALL_TYPE_RIGHT_LOOP);
	m_pMeshWall[5]->BindTexture(CManager::GetTexture()->GetTexture(CTexture::TEX_TYPE_WALL_FOREST));
	...
	for (int nCntGameObj = 0; nCntGameObj < 2; nCntGameObj++)
	{
		if (m_pGameObj[nCntGameObj] == NULL)
		{
			m_pGameObj[nCntGameObj] = CGameObject::Create(D3DXVECTOR3(GAME_PLAYER_START + nCntGameObj * 1200.0f, 0.0f, 0.0f), D3DXVECTOR3(1200.0f, 0.0f, 0.0f), 0);
		}
	}
...
void CTutorial::TimeLine(void)
{
	...
	if ((nPage == 6 && m_nTime % 8 == 0) || (nPage == 7 && m_nTime % 8 == 0))
	{
		// ホバリングさせる
