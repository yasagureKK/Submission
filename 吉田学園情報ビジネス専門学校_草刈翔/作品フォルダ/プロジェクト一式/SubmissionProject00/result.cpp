...
HRESULT CResult::Init(D3DXVECTOR3 pos, D3DXVECTOR3 size)
{
	...
	// 背景
	m_pBillboard = CBillboard::Create(D3DXVECTOR3(0.0f, 0.0f, 230.0f), D3DXVECTOR3(SCREEN_WIDTH, SCREEN_HEIGHT, 0.0f), D3DXVECTOR3(0.0f, 0.0f, 0.0f), PRIORITY_NONE);
	m_pBillboard->BindTexture(CManager::GetTexture()->GetTexture(CTexture::TEX_TYPE_RESULT_BG));
	...
	if (CManager::GetBeforeMode() == CManager::MODE_GAME)
	{
		...
		// プレイヤー
		m_pPlayer[0] = CPlayer::Create(D3DXVECTOR3(0.0f, 0.0f, 0.0f), D3DXVECTOR3(0.0f, D3DX_PI, 0.0f), 0);
	...
	if (CManager::GetBeforeMode() == CManager::MODE_PVP)
	{
		...
		// プレイヤー
		m_pPlayer[0] = CPlayer::Create(D3DXVECTOR3(-50.0f, 0.0f, 0.0f), D3DXVECTOR3(0.0f, D3DX_PI, 0.0f), 0);
		m_pPlayer[1] = CPlayer::Create(D3DXVECTOR3(50.0f, 0.0f, 0.0f), D3DXVECTOR3(0.0f, D3DX_PI, 0.0f), 1);
		...
		if (n1PScore > n2PScore)
		{
			...
			// 「かち！」の文字を表示
			m_apUi[7] = CUi::Create(D3DXVECTOR3(RESULT_UI_1P_POS_X + RESULT_UI_1P_SIZE_X, RESULT_UI_1P_POS_Y, 0.0f),
		...
		else if (n2PScore > n1PScore)
		{
			...
			// 「かち！」の文字を表示
			m_apUi[7] = CUi::Create(D3DXVECTOR3(SCREEN_WIDTH - RESULT_UI_1P_POS_X + RESULT_UI_1P_SIZE_X, RESULT_UI_1P_POS_Y, 0.0f),
				D3DXVECTOR3(RESULT_UI_WIN_SIZE_X, RESULT_UI_1P_SIZE_Y, 0.0f), PRIORITY_UI, CUi::UI_TYPE_RESULT_WIN);
