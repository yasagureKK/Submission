...
{
	if (m_type == MESHFIELD_TYPE_RIGHT_LOOP || m_type == MESHFIELD_TYPE_RIGHT_LOOP_START)
	{
		// プレイヤーを追尾
		TrackingPlayer();
	}
...
// 生成処理
//=============================================================================
CMeshField* CMeshField::Create(D3DXVECTOR3 pos, D3DXVECTOR3 size, D3DXVECTOR3 rot, int nRow, int nLine, MESHFIELD_TYPE type)
{
	// インスタンスの生成
	...
	while (pObject != NULL)
	{
		...
		if (pObject->GetObjType() == CScene::OBJECT_TYPE_PLAYER)
		{
			...
			if (m_nNum == pPlayer->GetNum())
			{
				...
				else if (m_type == MESHFIELD_TYPE_RIGHT_LOOP_START)
				{
					if (pos.x < 0 && pos.x > m_pos.x + m_size.x)
					{
						m_pos.x += m_size.x * 2;
					}
				}
