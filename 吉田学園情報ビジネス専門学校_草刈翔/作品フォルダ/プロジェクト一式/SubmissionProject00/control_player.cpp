...
void CControlPlayer::Update(CScene *pScene)
{
	...
	if (m_move.y < -PLAYER_GRAVITY && pPlayer->GetJump() == true && pPlayer->GetLand() == false &&
		pPlayer->GetMotionPlayer()->GetMotion() != CMotionPlayer::MOTION_PLAYER_TYPE_DOUBLE_JUMP &&
		pPlayer->GetMotionPlayer()->GetConnect() == false)
	{
		// 落下モーションにする
		...
		(m_move.x >= 1.0f || m_move.x <= -1.0f || m_move.z >= 1.0f || m_move.z <= -1.0f))
	{
		...
		if (m_nMoveCount % 10 == 0)
		{
			...
			// 土煙の生成
			CBillboardSmoke::Create(D3DXVECTOR3(pos.x, pos.y, pos.z), D3DXVECTOR3(70.0f, 70.0f, 0.0f), 20);
		}
	}
	else
	{
	...
	if (pPlayer->GetMotionPlayer()->GetMotion() == CMotionPlayer::MOTION_PLAYER_TYPE_DOUBLE_JUMP)
	{
		...
		if (m_nSpinCount >= 8)
		{
			...
			pSound->ControllVoice(CSound::SOUND_LABEL_SPIN, 0.4f);
			m_nSpinCount = 0;
		}
	}
	else
	...
	// チュートリアルで説明中ではない場合
	if (CTutorial::GetExplanation() == false)
	{
		// 計測が終わっていないなら
	...
	for (int nCntPlayer = 0; nCntPlayer < MAX_PLAYER_NUM; nCntPlayer++)
	{
		...
		if (pPlayer->GetLand() == true)
		{
			...
			if (CTutorial::GetExplanation() == false)
			{
				...
				//***********************************************************************
				// 下を連打でダッシュ (1P:S、2P:↓ または パッドXボタン)
				//***********************************************************************
				if (pInputKeyboard->GetTrigger(nDown) == true ||
					pInputPad->GetButtonTrigger(XINPUT_GAMEPAD_X, nNumPlayer) == true)
				{
					// SEの再生
					pSound->Play(CSound::SOUND_LABEL_ACCELERATION);
					pSound->ControllVoice(CSound::SOUND_LABEL_ACCELERATION, 0.4f);
			...
			if (pPlayer->GetStop() == false)
			{
				...
				if (nNumPlayer == nCntPlayer)
				{
						...
						pInputPad->GetButtonTrigger(XINPUT_GAMEPAD_A, nNumPlayer) == true)
					{
						...
						D3DXVECTOR3 pos = pPlayer->GetPos();
						// パーティクルで円形のエフェクトを出す
						m_pParticle = CParticleCircle::Create(pos, D3DXVECTOR3(20.0f, 7.0f, 0.0f), D3DXVECTOR3(0.0f, 0.0f, 0.0f),
...
void CControlPlayer::CommandMove(CPlayer *pPlayer)
{
	...
	case CPlayer::PLAYER_COMMAND_AIR_DASH:
		// 移動速度を上げる
		m_fSpeed += 0.0002f;
		break;
	...
	case CPlayer::PLAYER_COMMAND_HOVERING:
		// 2段ジャンプをしているかつ、移動量が0より下なら
		if (m_bDoubleJump == true && m_move.y < 0.0f)
		{
			// 浮力を増やす
			m_fFloat += 0.1f;
