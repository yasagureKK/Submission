...
public:
	...
	typedef enum
	{
		...
		FADE_IN,			// フェードイン
		FADE_OUT,			// フェードアウト
		FADE_FLASH_IN,		// フラッシュイン(モード遷移なし)
		FADE_FLASH_OUT,		// フラッシュアウト(モード遷移なし)
		FADE_SCREEN_COL_SET,// 画面カラー設定(モード遷移なし)
		FADE_MAX
	} FADE;
	...
	void SetFlash(D3DXCOLOR col);							// フラッシュ設定処理(モード遷移なし)
	void SetScreenCol(D3DXCOLOR col);						// 画面カラー設定処理(モード遷移なし)
